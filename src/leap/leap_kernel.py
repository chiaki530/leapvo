    ...
    def forward_iteration(
        ...
        for __ in range(iters):
            ...
            if self.cfg.kernel_block.add_time:
                times_embed_kernel = (
                    torch.from_numpy(get_1d_sincos_pos_embed_from_grid(kernel_feat.shape[-1], times_[0]))[None]
                    .repeat(B, 1, 1)
                    .float()
                    .to(device)
                )
    ...
    def forward(self, rgbs, queries, iters=4, feat_init=None, is_train=False):
        ...
        while ind < T - self.S // 2:
            ...
            if self.anchor_aug is not None and is_train:
                ...
                coords_init_anchor = repeat(anchor_queries[:,:,1:], 'b n c -> b s n c', s=S)
                vis_init_anchor = torch.ones((B, S, N_anchor, 1), device=device).float() * 10
                ...
                anchor_task_mask = (anchor_ind_array >= anchor_frame_id[None,None,:]).unsqueeze(-1)
                
                coords_init_all = torch.cat([coords_init_[:, :, :wind_idx], coords_init_anchor], dim=2)
                feat_init_all = torch.cat([feat_init[:, :, :wind_idx], feat_init_anchor], dim=2)
                vis_init_all = torch.cat([vis_init_[:, :, :wind_idx], vis_init_anchor], dim=2)
                track_mask_all = torch.cat([track_mask_[:, ind : ind + self.S, :wind_idx], anchor_task_mask], dim=2)
            ...
            if is_train:
                ...
                for cov_mat in covs:
                    cov_predictions_temp.append([cov_mat[0][:,:,:S_local, :S_local], cov_mat[1][:,:,:S_local, :S_local]])
            ...
            cov_x_e[:, ind : ind + self.S, :wind_idx] = torch.diagonal(covs[-1][0][:,:,:S_local, :S_local], dim1=-2, dim2=-1).permute(0,2,1)
            cov_y_e[:, ind : ind + self.S, :wind_idx] = torch.diagonal(covs[-1][1][:,:,:S_local, :S_local], dim1=-2, dim2=-1).permute(0,2,1)
        ...
        train_data = (
            (vis_predictions, coord_predictions, dynamic_predictions, cov_predictions, wind_inds, sort_inds)
            if is_train
            else None
        )
