...
def main(cfg: DictConfig):
    ...
    if os.path.isdir(imagedir):
        if cfg.data.traj_format == 'sintel':
            ...
            dataloader = dataset_stream(imagedir, calib, stride, skip, mode=cfg.data.traj_format)
    ...
    if gt_traj is not None:
        if cfg.data.traj_format in ['tum', 'tartanair']:
            pred_traj[1] = update_timestamps(cfg.data.gt_traj, cfg.data.traj_format, cfg.data.skip, cfg.data.stride)
