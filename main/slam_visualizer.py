...
class SLAMVisualizer:
    ...
    def draw_tracks_on_frames(self):
        ...
        for t, track in enumerate(self.tracks):
            ...
            for s in range(S):
                color = np.array(self.color_map(((t-S+1+s) % T)/ T)[:3])[None] * 255
...
class LEAPVisualizer(SLAMVisualizer):
            ...
            for i in range(N):
                ...
                if coord[0] != 0 and coord[1] != 0:
                    ...
                    if variances is not None:
                        ...
                        alpha = 0.5
                        res_video[t] = cv2.addWeighted(overlay, alpha, res_video[t], 1 - alpha, 0)
    ...
    def draw_tracks_on_frames(self):
        ...
        for t, track in enumerate(self.tracks):
            ...
            dyn_var = variances[:,:,high_mask].detach().cpu().numpy()  if variances is not None else None
