...
class MotionLabelMLPV1(nn.Module):
    def __init__(self, in_dim=128, hidden_dim=128, S=8):
        ...
        approx_gelu = lambda: nn.GELU(approximate="tanh")
        self.mlp = Mlp(
            in_features=self.in_dim, hidden_features=hidden_dim, out_features=1,
...
class MotionLabelATTN(nn.Module):
    def __init__(self, in_dim=128, num_heads=4):
        ...
        self.num_heads = num_heads
        approx_gelu = lambda: nn.GELU(approximate="tanh")
        self.self_attn = nn.MultiheadAttention(embed_dim=in_dim, num_heads=num_heads, batch_first=True)
        self.mlp = Mlp(
            in_features=self.in_dim, hidden_features=in_dim, out_features=1,
            act_layer=approx_gelu, drop=0
        )
...
class MotionLabelATTNV1(nn.Module):
    def __init__(self, in_dim=128, hidden_dim=128, num_heads=4, mlp_ratio=2.0, add_coord=False):
        super().__init__()
...
class MotionLabelBlock(nn.Module):
    def __init__(self, cfg, S):
        ...
        if self.cfg.mode == 'mlp':
            self.network = MotionLabelMLP(in_dim = self.cfg.in_dim, hidden_dim=self.cfg.hidden_dim, S=S)
        elif self.cfg.mode == 'mlp_v1':
            self.network = MotionLabelMLPV1(in_dim = self.cfg.in_dim, hidden_dim=self.cfg.hidden_dim, S=S)
        elif self.cfg.mode == 'mlp_v2':
            self.network = MotionLabelMLPV2(in_dim = self.cfg.in_dim, hidden_dim=self.cfg.hidden_dim)
        elif self.cfg.mode == 'attn':
            self.network = MotionLabelATTN(in_dim = self.cfg.in_dim, num_heads=self.cfg.num_heads)
        elif self.cfg.mode == 'attn_v1':
            self.network = MotionLabelATTNV1(in_dim = self.cfg.in_dim, hidden_dim=self.cfg.hidden_dim, num_heads=self.cfg.num_heads, mlp_ratio=self.cfg.mlp_ratio, add_coord=self.cfg.add_coord)
        else:
